...
- **Failed `update` leaves the lockfile unchanged.** `update` clears the lockfile to force a fresh resolution, but the original bytes are held in memory and restored if resolution or installation then fails. The same guarantee as a failed install, for the one command that deliberately destroys good pins before it knows it can produce new ones. A project that had no lockfile still has none afterwards.
